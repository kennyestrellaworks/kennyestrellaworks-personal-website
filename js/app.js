const DOMStrings = {
  contentProjectsItemsPrimaryBox: document.querySelector(
    ".content__projects-items-primary-box",
  ),
  contentProjectsItemsSecondaryBox: document.querySelector(
    ".content__projects-items-secondary-box",
  ),
  ...
  contentProfileWrapContent: document.querySelector(
    ".content__profile-wrap-content",
  ),
...
class UI {
  ...
  static setProjectsSectionWidth(browserWidth) {
    ...
    if (browserWidth <= 1270) {
      ...
      contentProjectsWrapWidth =
        LayoutSetup.containerWidthEdit -
        LayoutSetup.contentProfileWrapContentWidthEdit;
                        ...
                        <div class="content__projects-item-overlay-buttons">
                            ...
                            ${
                              data[i].videoDemo
                                ? `
                            <button class="button-2">
                                <a class="button-2__text" href="${data[i].videoDemo}" target="_blank">                                    

                                    <svg class="button-icon button-icon__live-demo" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" fill="#000000" width="34" height="34" version="1.1" id="Layer_1" viewBox="0 0 496.004 496.004" xml:space="preserve">
<g>
	<g>
...
<g>
    ...
    DOMStrings.contentProjectsItemsIcons = document.querySelectorAll(
      ".content__projects-items-icons",
    );
    DOMStrings.contentProjectsItem = document.querySelectorAll(
      ".content__projects-item",
    );
    DOMStrings.contentProjectsItemOverlay = document.querySelectorAll(
      ".content__projects-item-overlay",
    );
...
class Events {
  static contactFormCloseButton() {
    DOMStrings.contactFormCloseButton.addEventListener("click", function () {
      UI.hideAndShowElement(
        DOMStrings.pageOverlay,
        "page-overlay__hide",
        "add",
      );
      UI.hideAndShowElement(
        DOMStrings.pageOverlay,
        "page-overlay__default",
        "remove",
      );
      UI.hideAndShowElement(
        DOMStrings.contactFormWrap,
        "contact-form__wrap-hide",
        "add",
      );
      UI.hideAndShowElement(
        DOMStrings.contactFormWrap,
        "contact-form__wrap-default",
        "remove",
      );
      UI.hideAndShowElement(DOMStrings.contentContactButton, "button-1", "add");
      UI.hideAndShowElement(
        DOMStrings.contentContactButton,
        "button-1__active",
        "remove",
      );
    });
  ...
  static closeProfileSection() {
    DOMStrings.contentProfileClose.addEventListener("click", function () {
      DOMStrings.contentShowProfileBox.classList.remove(
        "content__show-profile-box-active",
      );
      UI.hideAndShowElement(
        DOMStrings.contentProfileWrapContent,
      ...
      UI.hideAndShowElement(
        DOMStrings.contentProfileClose,
  ...
  static contentShowProfileBoxButton() {
    DOMStrings.contentShowProfileBox.addEventListener("click", function () {
      DOMStrings.contentShowProfileBox.classList.add(
        "content__show-profile-box-active",
      );
      UI.hideAndShowElement(
        DOMStrings.contentProfileClose,
  ...
  static projectItemHover() {
    for (let i = 0; i < DOMStrings.contentProjectsItem.length; i++) {
      DOMStrings.contentProjectsItem[i].addEventListener(
        "mouseover",
        function () {
          DOMStrings.contentProjectsItemOverlay[i].classList.remove(
            "content__projects-item-overlay-hide",
          );
          DOMStrings.contentProjectsItemOverlay[i].classList.add(
  ...
  static windowResize() {
    window.addEventListener("resize", function () {
      ...
      if (
        !DOMStrings.contentShowProfileBox.classList.contains(
          "content__show-profile-box-active",
        )
      ) {
      ...
      if (
        ...
        if (browserWidth > 1270) {
          ...
          DOMStrings.contentProfileClose.classList.add(
            "content__profile-close-hide",
          );
          DOMStrings.contentProfileClose.classList.remove(
            "content__profile-close-default",
          );
        }
      }
...
class Process {
  static hideProfileSection(browserWidth) {
    if (browserWidth <= 1270) {
      DOMStrings.contentProfileWrapContent.classList.remove(
        "content__profile-wrap-content-default",
      );
      DOMStrings.contentProfileWrapContent.classList.add(
        "content__profile-wrap-content-hide",
      );
      DOMStrings.contentShowProfile.classList.add(
        "content__show-profile-default",
      );
      DOMStrings.contentShowProfile.classList.remove(
  ...
  static async getProjects() {
    try {
      ...
      projects = projects.map((product) => {
        ...
        const { headingColored, headingExtra, category, blurb } =
          product.fields;
        const {
          image: {
            fields: {
        ...
        const { liveDemo, sourceCode, videoDemo } = product.fields;
        return {
          id,
          headingColored,
          headingExtra,
